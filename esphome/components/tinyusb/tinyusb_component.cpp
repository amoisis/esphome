...
void TinyUSB::setup() {
  ...
  if (result != ESP_OK) {
    ESP_LOGE(TAG, "tinyusb_driver_install failed with error code: 0x%x (%s)", result, esp_err_to_name(result));
    // Only mark as failed if it's a critical error, not just "already initialized"
    if (result != ESP_ERR_INVALID_STATE) {
      // For non-critical errors, log but don't fail - USB may connect later
      ESP_LOGW(TAG, "TinyUSB initialization issue, but continuing - device may work on reconnection");
    } else {
      this->mark_failed();
    }
  } else {
